...
import zipfile
import ntpath
from resources.libs import extras
...
def Archive_Tree(sourcefile, destfile, message_header, message1, message2, message3, exclude_dirs, exclude_files):
    ...
    for base, dirs, files in os.walk(sourcefile):
        ...
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'plugin.program.btnewwizard' in dirs:
                   import time
                   FORCE= '01/01/1980'
...
def Archive_File(sourcefile, destfile):
    ...
    for_progress = []
    ITEM =[]
    dp.create("[COLOR=white]Brokentechie Wizard[/COLOR]","Archiving...",'', 'Please Wait')
    for base, dirs, files in os.walk(sourcefile):
        for file in files:
            ITEM.append(file)
    N_ITEM =len(ITEM)
...
#Create backup menu
def Backup_Option():
    extras.addDir('','[COLOR=yellow]Full Backup[/COLOR]','url','community_backup','Brokentechie_Builds.png','','','Back Up Your Full System')
    extras.addDir('','Backup Just Your Addons','addons','restore_zip','Brokentechie_Builds.png','','','Back Up Your Addons')
    extras.addDir('','Backup Just Your Addon UserData','addon_data','restore_zip','Brokentechie_Builds.png','','','Back Up Your Addon Userdata')
    extras.addDir('','Backup Guisettings.xml',GUI,'restore_backup','Brokentechie_Builds.png','','','Back Up Your guisettings.xml')
    if os.path.exists(FAVS):
        extras.addDir('','Backup Favourites.xml',FAVS,'restore_backup','Brokentechie_Builds.png','','','Back Up Your favourites.xml')
    if os.path.exists(SOURCE):
        extras.addDir('','Backup Source.xml',SOURCE,'restore_backup','Brokentechie_Builds.png','','','Back Up Your sources.xml')
    if os.path.exists(ADVANCED):
        extras.addDir('','Backup Advancedsettings.xml',ADVANCED,'restore_backup','Brokentechie_Builds.png','','','Back Up Your advancedsettings.xml')
    if os.path.exists(KEYMAPS):
        extras.addDir('','Backup Keyboard.xml',KEYMAPS,'restore_backup','Brokentechie_Builds.png','','','Back Up Your keyboard.xml')
...
def Check_Download_Path():
    ...
    if not os.path.exists(zip):
        dialog.ok('[COLOR=white]Brokentechie Wizard[/COLOR]','The download location you have stored does not exist .\nPlease update the addon settings and try again.','','')        
...
def Community_Backup():
    ...
    exclude_dirs_full =  ['plugin.program.btnewwizard']
    exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf']
    exclude_dirs =  ['plugin.program.btnewwizard','cache', 'system', 'Thumbnails', "peripheral_data"]
    ...
    choice = xbmcgui.Dialog().yesno("This backup includes your addon_data folder", 'This contains ALL addon settings including passwords.', 'If you\'re intending on sharing this with others it is', 'recommended to manually remove data from the generated zip file.', yeslabel='OK',nolabel='STOP')
    if choice == 0:
        return
    elif choice == 1:
        pass
...
def Fix_Special(url):
    dp.create("[COLOR=white]Brokentechie Wizard[/COLOR]","Renaming paths...",'', 'Please Wait')
...
def Restore_Option():
    if trcheck == 'true':
        Check_Local_Install()
    extras.addDir('','[COLOR=lime]RESTORE LOCAL BUILD[/COLOR]','url','restore_local_CB','Brokentechie_Builds.png','','','Back Up Your Full System')
    extras.addDir('','[COLOR=dodgerblue]Restore Local guisettings file[/COLOR]','url','LocalGUIDialog','Brokentechie_Builds.png','','','Back Up Your Full System')
    
    if os.path.exists(os.path.join(USB,'addons.zip')):   
        extras.addDir('','Restore Your Addons','addons','restore_zip','Brokentechie_Builds.png','','','Restore Your Addons')

    if os.path.exists(os.path.join(USB,'addon_data.zip')):   
        extras.addDir('','Restore Your Addon UserData','addon_data','restore_zip','Brokentechie_Builds.png','','','Restore Your Addon UserData')           

    if os.path.exists(os.path.join(USB,'guisettings.xml')):
        extras.addDir('','Restore Guisettings.xml',GUI,'restore_backup','Brokentechie_Builds.png','','','Restore Your guisettings.xml')
    
    if os.path.exists(os.path.join(USB,'favourites.xml')):
        extras.addDir('','Restore Favourites.xml',FAVS,'restore_backup','Brokentechie_Builds.png','','','Restore Your favourites.xml')
        
    if os.path.exists(os.path.join(USB,'sources.xml')):
        extras.addDir('','Restore Source.xml',SOURCE,'restore_backup','Brokentechie_Builds.png','','','Restore Your sources.xml')
        
    if os.path.exists(os.path.join(USB,'advancedsettings.xml')):
        extras.addDir('','Restore Advancedsettings.xml',ADVANCED,'restore_backup','Brokentechie_Builds.png','','','Restore Your advancedsettings.xml')        

    if os.path.exists(os.path.join(USB,'keyboard.xml')):
        extras.addDir('','Restore Keyboard.xml',KEYMAPS,'restore_backup','Brokentechie_Builds.png','','','Restore Your keyboard.xml')
        
    if os.path.exists(os.path.join(USB,'RssFeeds.xml')):
        extras.addDir('','Restore RssFeeds.xml',RSS,'restore_backup','Brokentechie_Builds.png','','','Restore Your RssFeeds.xml')    
#---------------------------------------------------------------------------------------------------
...
def Restore_Zip_File(url):
    ...
    if 'Backup' in name:
        Delete_Packages() 
        dp.create("[COLOR=white]Brokentechie Wizard[/COLOR]","Backing Up",'', 'Please Wait')
        ...
        zipobj.close()
        dp.close()
        dialog.ok("[COLOR=white]Brokentechie Wizard[/COLOR]", "You Are Now Backed Up", '','')   
    else:
