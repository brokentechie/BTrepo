...
def CATEGORIES():
    link = OPEN_URL('http://brokentechie.ddns.net/wizard.txt').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    addDir('Fresh Start','', 2, 'http://brokentechie.ddns.net/FreshStart.png', 'http://brokentechie.ddns.net/fanart.jpg','Perform Fresh Start')
...
if mode == 2:
    freshstart()
    dialog = xbmcgui.Dialog()
    dialog.ok("Fresh Start complete")
    killxbmc()
