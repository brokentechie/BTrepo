...
def Archive_Tree(sourcefile, destfile, message_header, message1, message2, message3, exclude_dirs, exclude_files):
    ...
    for base, dirs, files in os.walk(sourcefile):
        ...
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'plugin.program.btwizard' in dirs:
                   import time
                   FORCE= '01/01/1980'
...
def Community_Backup():
    ...
    exclude_dirs =  ['plugin.program.btwizard','cache', 'system', 'Thumbnails', "peripheral_data",'library']
...
def Restore_Local_Community():
    ...
    if exitfunction == 1:
        ...
        dialog.ok('[COLOR=white]Brokentechie Wizard[/COLOR]','Step 1 complete. Now please change the skin to','the one this build was designed for. Once done come back','to this addon and restore the guisettings_fix.zip')        
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")
#---------------------------------------------------------------------------------------------------
